...
void Machine_init(){
    ...
    ACM.Lmu    = IM_INVERSE_GAMMA_CIRCUIT_MAGNETIZING_INDUCTANCE;
        // Those parameters are introduced since branch saturation
        ACM.Lls = IM_T_CIRCUIT_STATOR_LEAKAGE_INDUCTANCE;
        ACM.Llr = IM_T_CIRCUIT_ROTOR_LEAKAGE_INDUCTANCE;
    ...
    ACM.rreq   = IM_INVERSE_GAMMA_CIRCUIT_ROTOR_RESISTANCE;
    ACM.rs     = IM_STATOR_RESISTANCE;
        ACM.rr = ACM.rreq * ACM.Lr_slash_Lm*ACM.Lr_slash_Lm;

    ACM.alpha  = ACM.rreq / (ACM.Lmu);
    ACM.Lmu_inv= 1.0/ACM.Lmu;

    ACM.Js = IM_SHAFT_INERTIA; // Awaya92 using im.omg
    ACM.npp = IM_NUMBER_OF_POLE_PAIRS;
...
int main(){
    ...
    for(_=0;_<NUMBER_OF_STEPS;++_){
        ...
        cmd_slow_speed_reversal(CTRL.timebase, 3, 50, 50); // timebase, instant, interval, rpm_cmd
...
void write_header_to_file(FILE *fw){
    ...
    fprintf(fw, "ACM.x[0],ACM.x[1],ACM.x[2],ACM.x[3],ACM.x[4],ACM.Tem,ACM.ual,ACM.ube,ACM.rpm_cmd,e_omega,tildeOmg,hatOmg,VarEpsilonAl,VarEpsilonBe\n");
    ...
    // if(CTRL.timebase>20)
    {
        if(++j == DOWN_SAMPLE)
        {
            ...
            fprintf(fw, "%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g\n",
                    ...
                    ACM.ual,ACM.ube,ACM.rpm_cmd,ACM.rpm_cmd-ACM.x[4]*RAD_PER_SEC_2_RPM, ACM.x[4]-OB_OMG, OB_OMG,
                    ob.mismatch[0], ob.mismatch[1]
                    );
        }
    }
